...
<div align="center">
  <img src="public/logo.png" alt="CKTV Logo" width="120">
</div>

> 🎬 **CKTV-传康播放器** 是一个开箱即用的跨平台影视聚合播放器，基于 MoonTV 二次开发。采用 **Next.js 14** + **Tailwind CSS** + **TypeScript** 构建，支持多资源搜索、在线播放、收藏同步、播放记录、本地/云端存储。
>
> 📱 **作者**：传康KK | Vx：1837620622 | 邮箱：2040168455@qq.com | 咸鱼/B站：万能程序员
...
- 🔍 **多源聚合搜索**：内置 **53个** 经过验证的免费资源站点，一次搜索返回全源结果
- 🗄️ **豆瓣数据集成**：首页展示热门电影、剧集、动漫、综艺、纪录片，内置图片代理绕过防盗链
- 📄 **丰富详情页**：支持剧集列表、演员、年份、简介等完整信息展示
- ▶️ **流畅在线播放**：集成 HLS.js & ArtPlayer 播放器
- ❤️ **收藏 + 继续观看**：支持 Redis/D1/Upstash 存储，多端同步进度
- 📱 **PWA 支持**：离线缓存、安装到桌面/主屏，移动端原生体验
- 🌗 **响应式布局**：桌面侧边栏 + 移动底部导航，自适应各种屏幕尺寸
- 🚀 **极简部署**：Docker 一键部署，或免费部署到 Vercel 和 Cloudflare
...
```bash
# 拉取预构建镜像
docker pull ghcr.io/1837620622/cktv:latest

# 运行容器
# -d: 后台运行  -p: 映射端口 3000 -> 3000
docker run -d --name cktv -p 3000:3000 --env PASSWORD=your_password ghcr.io/1837620622/cktv:latest
```
...
services:
  cktv:
    image: ghcr.io/1837620622/cktv:latest
    container_name: cktv
...
services:
  cktv-core:
    ...
    environment:
      ...
      - REDIS_URL=redis://cktv-redis:6379
...
| SITE_NAME                         | 站点名称                                     | 任意字符串                       | CKTV-传康播放器                                                                                                                     |
...
CKTV-传康播放器 支持标准的苹果 CMS V10 API 格式。
...
### 2025-01-20
- 📺 **电视投屏功能**：新增三种投屏方式
  - 投屏：基于 W3C Remote Playback API（支持 Chromecast/AirPlay）
  - 扫码：生成二维码，电视浏览器扫码观看
  - 链接：复制页面/视频链接到电视浏览器打开
- 🎨 **投屏 UI 优化**：标签页切换、连接状态显示、加载动画
- 📍 **投屏按钮位置**：移至播放器上方控制栏

### 2025-01-19
- ✨ **API接口扩展**：新增29个有效资源站点，总计53个
- 🎭 **导航优化**：新增动漫、纪录片独立导航项
- 🔧 **豆瓣分类**：支持URL预设子分类参数（sub参数）
- 🐛 **修复**：导航激活状态判断逻辑优化
...
[MIT](LICENSE) © 2025 CKTV-传康播放器 & 传康KK

基于 [MoonTV](https://github.com/senshinya/MoonTV) 二次开发
